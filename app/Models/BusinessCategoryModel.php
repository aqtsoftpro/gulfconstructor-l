...
class BusinessCategoryModel extends BaseModel
{
    protected $builder;
    ...
    {
        parent::__construct();
        $this->builder = $this->db->table('business_category');
    }
    ...
    public function buildQuery($allColumns = true)
    {
        ...
        if ($allColumns == true) {
            $this->builder->select('business_category.*, business_category.parent_id AS join_parent_id');
        } else {
            $this->builder->select('business_category.id, business_category.slug, business_category.parent_id, business_category.parent_tree, business_category.category_order, business_category.featured_order, business_category.storage, business_category.image, business_category.show_on_main_menu, business_category.show_image_on_main_menu, business_category.parent_id AS join_parent_id');
        }
        $this->builder->select("(SELECT GROUP_CONCAT(lang_id, '" . CAT_QUERY_SEPARATOR_SUB . "', name SEPARATOR '" . CAT_QUERY_SEPARATOR . "') FROM business_category_lang WHERE business_category_lang.category_id = business_category.id) AS name")
            ->select('(SELECT slug FROM business_category WHERE id = join_parent_id) AS parent_slug');
    }
    ...
    public function getSubLevelCategoryIds($array, $numChild)
    {
        ...
        $sql = "SELECT  SubCategory FROM (
            ...
            JOIN business_category AS parent ON child.parent_id = parent.id 
            JOIN business_category_lang AS child_lang ON child.id = child_lang.category_id
    ...
    {
        $cacheKey = 'cat_cache_business_parent_categories';
    ...
    {
        // $cacheKey = "cat_cache_business_categories";
        // $arrayCache = getCacheStatic($cacheKey);
        $this->buildQuery();
        $this->builder->where('business_category.parent_id', clrNum($parentId));
        $this->orderByCategories();
        $rows = $this->builder->get()->getResult();
    ...
    {
        $this->buildQuery();
        return $this->builder->where('business_category.id', clrNum($id))->get()->getRow();
    }
    ...
    public function getCategoryBySlug($slug, $type = 'all')
    {
        ...
        return $this->builder->where('visibility', 1)->where('business_category.slug', cleanStr($slug))->get()->getRow();
    }
    ...
    public function getVendorCategoriesTree($vendorId = null, $category = null, $onlyHasProducts = true, $returnByParentId = true)
    {
        ...
        if ($onlyHasProducts == true) {
            if (!empty($vendorId)) {
                $this->builder->where('business_category.id IN (SELECT category_id FROM products WHERE products.status = 1 AND products.visibility = 1 AND products.is_draft = 0 AND products.is_deleted = 0 AND user_id = ' . clrNum($vendorId) . ')');
            } else {
                $this->builder->where('business_category.id IN (SELECT category_id FROM products WHERE products.status = 1 AND products.visibility = 1 AND products.is_draft = 0 AND products.is_deleted = 0)');
            }
        }
        if (!empty($category)) {
            $this->builder->where('FIND_IN_SET(' . $category->id . ', business_category.parent_tree)');
        }
    ...
    {
        return $this->builder->select('business_category.*')
            ->select("(SELECT GROUP_CONCAT(lang_id, '" . CAT_QUERY_SEPARATOR_SUB . "', name SEPARATOR '" . CAT_QUERY_SEPARATOR . "') FROM business_category_lang WHERE business_category_lang.category_id = business_category.id) AS name")
            ->where('business_category.id IN (SELECT category_id FROM products WHERE products.status = 1 AND products.visibility = 1 AND products.is_draft = 0 AND products.is_deleted = 0 AND user_id = ' . clrNum($vendorId) . ')')->get()->getResultArray();
    }
    ...
    public function getCategoryParentTree($category, $onlyVisible = true)
    {
        ...
        $this->buildQuery();
        $this->builder->whereIn('business_category.id', $ids, false);
        if ($onlyVisible == true) {
            $this->builder->where('business_category.visibility', 1);
        }
    ...
    public function getCategoriesByIdArray($array)
    {
        if (!empty($array) && countItems($array) > 0) {
            ...
            if (!empty($ids) && countItems($ids) > 0) {
                $this->buildQuery();
                return $this->builder->whereIn('business_category.id', $ids, false)->get()->getResult();
            }
        }
        return array();
    }
    ...
    public function orderByCategories()
    {
        $sort = $this->generalSettings->sort_categories;
        if ($sort == 'date') {
            $this->builder->orderBy('business_category.created_at');
        } elseif ($sort == 'date_desc') {
            $this->builder->orderBy('business_category.created_at', 'DESC');
        } elseif ($sort == 'alphabetically') {
    ...
    public function addCategoryName($categoryId)
    {
        foreach ($this->activeLanguages as $language) {
            ...
            $this->db->table('business_category_lang')->insert($data);
        }
    }
    ...
    {
        $category = $this->getBusinessCategory($id);
        if (!empty($category)) {
    ...
    public function updateCategoryName($categoryId)
    {
        foreach ($this->activeLanguages as $language) {
            ...
            $row = $this->db->table('business_category_lang')->where('category_id', clrNum($categoryId))->where('lang_id', $language->id)->get()->getRow();
            if (empty($row)) {
                $this->db->table('business_category_lang')->insert($data);
            } else {
                $this->db->table('business_category_lang')->where('category_id', clrNum($categoryId))->where('lang_id', $language->id)->update($data);
            }
        }
    }
    ...
    {
        if (!empty($category)) {
            $categories = $this->builder->select('business_category.id, business_category.parent_id AS parent_category_id, (SELECT parent_tree FROM business_category WHERE id = parent_category_id) AS parent_category_tree')
    ...
    public function getBusinessCategoriesSearchCount()
    {
    ...
    public function getBusinessCategoriesSearchPaginated($perPage, $offset)
    {
        $this->filterCategories();
        return $this->builder->orderBy('business_category.created_at DESC')->limit($perPage, $offset)->get()->getResult();
    }
    ...
    private function filterCategories()
    {
        ...
        if (!empty($q)) {
            $this->builder->like('business_category_lang.name', $q);
        }
        $this->builder->select('business_category.*, business_category_lang.name as name')->join('business_category_lang', 'business_category_lang.category_id = business_category.id')
            ->where('business_category_lang.lang_id', selectedLangId());
    }
            ...
            foreach ($array as $item) {
                if ($i == $index) {
                    ...
                    if ($this->builder->insert($data)) {
                        ...
                        $this->db->table('business_category_lang')->insert($dataName);
                        //update slug
                        $this->updateSlug($lastId);
    ...
    {
        $category = $this->getBusinessCategory($categoryId);
        if (!empty($category)) {
    ...
    public function deleteBusinessCategoryImage($categoryId)
    {
        $category = $this->getBusinessCategory($categoryId);
        if (!empty($category)) {
            deleteFile($category->image);
    ...
    public function deleteBusinessCategory($id)
    {
        $category = $this->getBusinessCategory($id);
        if (!empty($category)) {
            $this->db->table('business_category_lang')->where('category_id', $category->id)->delete();
